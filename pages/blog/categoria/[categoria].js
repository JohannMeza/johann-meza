...
export default function CategoriaPage({ listPublicaciones, listEtiquetas, listCategorias, categoriaSelect, listLastPublicaciones, dataPaginate }) {
  ...
  const handleClickEtiquetas = (idEtiquetas) => {
    setEtiquetasSelect(etiquetas => {
      ...
      getPublicaciones(listaEtiquetas, { ...paginate, page: 0 })
      navigation.current.state.selected = 0;
  ...
  return (
    <BodyComponent>
      <div className={frontStyles.blogBanner}>
        <h1 className="text-title-2 lg:text-[62px] text-start text-white font-Poppins font-extrabold">
...
export async function getServerSideProps({ query }) {
  ...
  const getCategoriaActual = async () => {
    await SendRequestData({
      queryId: 68,
      body: { SLUG: query.categoria },
      success: (resp) => (categoriaSelect = resp.dataObject),
  ...
  const getPublicaciones = async () => {
    await SendRequestData({
      queryId: 34,
      body: { ID_ETIQUETAS: '', SLUG: query.categoria },
